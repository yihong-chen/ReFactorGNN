"""A demo of ReFactor GNN induced by DistMult optimised with SGD over a cross-entropy loss
...
class ReFactorConv(MessagePassing):
    def __init__(self, n_ent=None, n_rel=None, n_hid=None, 
        ...
        self.fm_alpha, self.fm_optim, self.fm_score_func = fm_alpha, fm_optim, fm_score_func
    ...
    def message_and_aggregate(self, adj, x=None):
        ...
        if self.fm_score_func == 'DistMult':
            ...
            aggr_other2v = scatter(src=msg_fit, index=v, dim=0, dim_size=n_nodes)
            aggr_out.append(aggr_other2v)
            '''Direction 2: v2w, z_{w<-v} = - grad_w'''
            msg_fit = (1 - Zw) * rp * hv
            aggr_v2neighbor = scatter(src=msg_fit, index=w, dim=0, dim_size=n_nodes)
            aggr_out.append(aggr_v2neighbor)
            '''Direction 2: v2u, z_{u<-v}= - grad_u'''
